    ...
    mod.event = function () {
        ...
        $('.customer').on('click', function () {
            ...
            if (JsVilaDataNull(mod.contactUrl) && mod.contactUrl.indexOf("http") >= 0) {
                ...
                window.location.href = webImUrl.format(contactAppId);
            } else {
                try {
                    if ($.os.ios) { //ios
    ...
    mod.initData = function (data) {
        ...
        if (data.SharePercent != null && parseFloat(data.SharePercent) > 0) {
            ...
            var html = '<span class="commission">' +
                ...
                '<a href="javascript:void(0);" class="pic"><img src="/Content/Mobile/wenhao.png"></a>' +
                '</span>' +
                '</span>';
            $('#d-adds').append(html);
        }
        ...
        if (data.Coupons && data.Coupons.length > 0) {
            $("#coupon-text").text(data.Coupons.join(",")).removeClass('hide');
            $('#coupon-wrap').removeClass('hide');
        } else {
            $("#coupon-text").addClass('hide');
        }
        /*设置优惠券领取列表*/
        if (data.CouponList && data.CouponList.length > 0) {
            ...
            data.CouponList.forEach(function (v) {
                couhtml +=
                    ...
                    '<div class="list">' +
                    (v.Used ? '<div class="list-left used">' : '<div class="list-left">') +
                    ...
                    (v.Used ? '<a class="btn disabled">已领取</a>' : '<a class="btn" onclick=\'DETAIL.bindCoupon("' + v.Id + '",this)\'>点击领取</a>') +
                    '</div>' +
                    '</div>' +
                    '</div>';
            });
        ...
        if (data.YJBInfo && data.YJBInfo.Enabled) {
            $("#yjb-text").text('可用' + data.YJBInfo.InsteadCashCount + '易捷币抵现' + data.YJBInfo.InsteadCashAmount + '元').removeClass('hide');
            $('#coupon-wrap').removeClass('hide');
        } else {
        ...
        if (couponEnable && yjbEnable && youkaEnable) {
            couponExtraText = "优惠券、易捷币、加油卡兑换券三种优惠不能同时享受";
        } else if (couponEnable && yjbEnable) {
            couponExtraText = "优惠券、易捷币两种优惠不能同时享受";
        } else if (couponEnable && youkaEnable) {
            couponExtraText = "优惠券、加油卡兑换券两种优惠不能同时享受";
        } else if (youkaEnable && yjbEnable) {
            couponExtraText = "易捷币、加油卡兑换券两种优惠不能同时享受";
        }
            ...
            if (mod.currentColor && mod.currentSize) { //两种属性已经选择了
                for (var i = 0; i < mod.commodityStocks.length; i++) {
                    ...
                    if ((commodity.ComAttribute[0].SecondAttribute == mod.currentColor && commodity.ComAttribute[1].SecondAttribute == mod.currentSize) || (commodity.ComAttribute[1].SecondAttribute == mod.currentColor && commodity.ComAttribute[0].SecondAttribute == mod.currentSize)) {
                        ...
                        if (mod.SkuActivityCdtos.length > 0) {
