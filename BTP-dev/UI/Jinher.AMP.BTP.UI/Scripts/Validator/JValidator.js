...
//整数
function isZhengNumber(s) {
    var regu = /^[1-9]*[1-9][0-9]*$/g //整数;
...
//用途：检查输入字符串是否符合金额格式
//格式定义为带小数的正数，小数点后最多九位
//输入：
//s：字符串
//返回：
//如果通过验证返回true,否则返回false
function isMoney(s) {
    var regu = /^[0-9]+[\.][0-9]{0,9}$/g;
    var re = new RegExp(regu);
    if (re.test(s)) {
        return true;
    } else {
...
function JudgeValidate(obj) {
    ...
    $(obj).find("[datacol=yes]").each(function () {
        ...
        if ($(this).attr("isSelect") && $(this).attr("isSelect") == "true") {
            ...
            if ($(this).attr("checkexpession") != undefined) {
                switch ($(this).attr("checkexpession")) {
                    ...
                    case "ZhengNum":
                        {
                            if (!isZhengNumber($(this).attr("value"))) {
...
function CheckInputFloatNullable(e) {
    var val = $.trim($(e).val());
    if (val == "") {
        return;
    }
