...
function PQLoadGrid(obj_ID, url, colM, sort, PageSize, topVisible) {
    ...
    var dataModel = {
        location: "remote",
        sorting: "", //原始为remote 采用服务器排序
        paging: "remote",
        dataType: "JSON",
        method: "GET",
        curPage: 1,
        rPP: PageSize,
        sortIndx: 1, //sortIndx 0为服务器排序 1为本地排序
        sortDir: "up",
...
function SetSystemId(MenuId) {
    //    getAjax("user.json", "action=SetSystemId&SystemId=" + MenuId, function (data) {
    //    })
}
...
function LoadAgain() {
    var tabs_container = top.$("#tabs_container");
    var tab_id = tabs_container.find('.selected').attr('id').substr(5);
    top.frames["tabs_iframe_" + tab_id + ""].window.windowload();
}
...
function myBrowser() {
    ...
    var isSafari = userAgent.indexOf("Safari") > -1; //判断是否Safari浏览
    if (isIE) {
...
//form  ajax 提交
/*url 提交地址 type 提交方式 data 数据  fnSuccess成功方法  fnError失败方法*/

function formAjaxPost(url, type, data, dataType, fnSuccess, fnError) {
    $.ajax({ url: url, type: type, dataType: dataType, data: data, success: fnSuccess, error: fnError });
    return false;
}
function formPost(form, fnSuccess, fnError) {
    Loading(true);
