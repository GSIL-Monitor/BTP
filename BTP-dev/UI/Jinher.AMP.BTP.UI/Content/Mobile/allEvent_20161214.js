...
function setItemData1(obj) {
    ...
    if (data.DiscountPrice > 0) {
        ...
        clone_item.find('.price_2').text(getCookie('Currency') + data.Price);
        clone_item.find('.zk').text('优惠价')
        isyou = true;
    }
    else {
        if (data.Intensity == 10) {
            clone_item.find('.price_1').text(getCookie('Currency') + data.Price);
...
//            }
    ...
    function setData(comDetailResult) {
        ...
        if (data.SharePercent != null
            ...
            $("#spanCommission .commission").html(getCookie('Currency') + commission);
            $("#spanCommission").show();
        } else {
            $("#spanCommission").hide();
        }
    ...
    function showDiyGroupDetail(data)
    {
       ...
       $("#spanPriceDiy").html(getCookie('Currency') + diyg.DiscountPrice);
       $("#btnDiyGroup div.open-group-price").html(getCookie('Currency') + diyg.DiscountPrice);
       

       $("#spanPriceOri").html(getCookie('Currency') + data.Price);
       $("#btnDirectBuy div.alone-buy-price").html(getCookie('Currency')+  data.Price);
        ...
        function normalEvent() {
            ...
            if (cmdtyProperties.children().length <= 0) {
                promotionTemplate = '<p class="u-detail-pricebox clearFloat">' +
                    '<span class="u-detail-price">'+getCookie('Currency')+'</span> <span id="minPrice" class="u-detail-price u-detail-price2">' +
                    '</span><span id="maxPrice" class="u-detail-price u-detail-price2"></span><span class="u-detail-mprice oldPrice">' +
                    '<span> '+getCookie('Currency')+'</span> <span id="minOldPrice"></span><span id="maxOldPrice"></span></span>' +
            ...
            if (footer.children("div").filter("#prmPresell").length !== 0) {
             ...
             if (cmdtyProperties.children("div").length > 0) {
                ...
                promotionTemplate = '<div style="padding-left: 0;  margin-bottom: .3rem;">' +
                    '<span style=" clear:both;">预售价：</span><span style="color: #FF0054;" class="yuan">'+getCookie('Currency')+'</span> <span id="minPrice" class="u-detail-price2" style="color: #FF0054;">' +
...
function calcFreight(hasAddress) {
//    if ($("#selftaketoggle").is(".active")) {
//        $('#freight').html("0.00");
//        setSessionStorage('commodityUpInfo', 'freight', 0.00);
//        CalcPayPrice();
//        return;
//    }
    //$("#selftaketoggle").is(".active") ? "1":"0";
    var sttoggle = isSelfTakeForCreateOrder(); 
    //var sttoggle = 0; //$("#selftaketoggle").is(".active") ? "1":"0";
    var str = "";
    ...
    if (sessionStorage.type == "gouwuche") {
        ...
        if (shopList && shopList.length) {
            for (var i = 0; i < shopList.length; i++) {
                ...
                {
                    var cj = clist[j];
                    subData.push({ AppId: cj.AppId, CommodityId: cj.Id, Count: cj.CommodityNumber, Price: cj.RealPrice });
                }
            }
        }
    } else {
...
//显示运费和优惠券信息。
function showFreightCouponInfo(result) {
    //显示运费信息。
    ...
    if (sessionStorage.type == "gouwuche") {
        ...
        $("#divShopCommoditys > div").each(function(ind, el) {
                ...
                if (cdto != null) {
                    ...
                    if (cdto.CouponType == 0) {
                        ...
                        if (cdto.Cash > totalPriceCommodity) {
                            ...
                            cspan.html(totalPriceCommodity);
                        } else {
                            appAmount -= cdto.Cash;
                            cspan.html(cdto.Cash);
                        }
                    }
                }
            }
            ...
            //合计
            $(this).find('ul li span[co-tag="AppAmount"]').html(appAmount);

            shop.RealPrice = appAmount;
        ...
        //订单需要支付的金额。
        var payAmount = Math.abs(commodityUpInfo.number * commodityUpInfo.realPrice).toFixed(2) * 1.0;

        //优惠券
        $("#spanCouponAmount").html("0");
        $("#spanCouponAmount").parent("li").hide();
        if (_couponResult != null && _couponResult.length > 0) { 
            //优惠券抵用金额
            var camount = payAmount > _couponResult[0].Cash ? _couponResult[0].Cash : payAmount;
            payAmount = payAmount - camount;
            $("#spanCouponAmount").html(camount);
            $("#spanCouponAmount").parent("li").show();
        } 
    }
    initScore();
...
function initScore()
{
     ...
     if(payPriceValue > 0 && _isShowScore && JsVilaDataNull(window._userScore))
        {
            var scoreValue = payPriceValue > window._userScore.Money ?  window._userScore.Money :payPriceValue;
            var scoreUsed = parseInt(scoreValue * window._userScore.ScoreCost);
            if(scoreUsed > 0)
            {
            ...
            { 
                ...
                //有积分，但订单用不了(scoreUsed =0 当前订单可用积分为0)。
                if(window._userScore.Money > 0)
                {
                    $("#spanActivedScore").hide();
                    $("#spanNoScore").hide();
                    $("#spanCannotScore").show();
                }
            } 
        }
        else
...
function numberEvent() {
    ...
    box.on("click",'.span_2', function() { //设置减事件,并更新缓存
        ...
        $('#pre_money_count').text(getCookie('Currency') + Math.abs(sessionStorage.Price * text).toFixed(2));
    ...
    $(".span_number").on("change", function() {
        ...
        $('#pre_money_count').text(getCookie('Currency') + Math.abs(sessionStorage.Price * v).toFixed(2));
...
//根据订单的当前状态显示可操作按钮列表。
function getButtonByState(data,state,pay)
{
     //最新请参见Model.CommodityOrder.
    //订单状态（必填）：未支付=0，未发货=1，已发货=2，确认收货=3，商家取消订单=4，客户取消订单=5，超时交易关闭=6，已退款=7，待发货退款中=8，已发货退款中=9,已发货退款中商家同意退款，商家未收到货=10,付款中=11,金和处理退款中=12,出库中=13，出库中退款中=14
        //永远不会“货到付款”
