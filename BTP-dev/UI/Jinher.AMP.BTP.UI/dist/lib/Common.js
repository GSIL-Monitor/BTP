...
* 获取订单状态 (订单列表)
*/
var getOrderStateTextForList = function (state, StateAfterSales) {
    //订单状态：未支付=0，未发货=1，已发货=2，确认收货=3，商家取消订单=4，客户取消订单=5，超时交易关闭=6，已退款=7，待发货退款中=8，已发货退款中=9,已发货退款中商家同意退款，商家未收到货=10,付款中=11,金和处理退款中=12,出库中=13，出库中退款中=14
    var text = "";
    switch (state) {
        case 0:
...
/*
URL公共处理方法
thiszpho2o=1    IOS区分打开webview用
viewby=0        页面打开方式：APP、WEB
active=finish   返回首页
entrance=esnet  馆馆互联列表中打开电商馆
*/
function RequestUrlParam() {
    var cjzy = getQueryString('cjzy');
    var vSrcTagId = getQueryString('SrcTagId');
    var vShareId = getQueryString('ShareId');
    var islogin = getQueryString('islogin');

    var userId = getQueryString('userId');
    var sessionId = getQueryString('sessionId');
    var changeOrg = getQueryString('changeOrg');

    var url = (initUrlParam.substring(0, 1) == "&" ? "" : "&") + initUrlParam;
    if (cjzy != null) {
        url += "&cjzy=" + cjzy;
    }
    if (vSrcTagId != null) {
        url += "&SrcTagId=" + vSrcTagId;
    }
    if (vShareId != null) {
        url += "&ShareId=" + vShareId;
    }
    if (userId != null) {
        url += "&userId=" + userId;
    }
