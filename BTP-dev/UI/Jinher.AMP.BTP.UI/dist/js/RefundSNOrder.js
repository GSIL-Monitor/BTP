...
function btnSubmit() {
    ...
    //判断寄件人信息是否填写 TODO
    var refundType = $('.type > .refund-btns a').attr('val');
    if (refundType != "10") {
        toast("抱歉该商品现在不能退货，支持换货或维修，请联系客服处理~");
        return false;
    }

    if (!defaultVue.selectProvinceCode || !defaultVue.selectCityCode || !defaultVue.selectDistrictCode) {
        toast("请选择取件地址~");
        return false;
    }
    if (!$("#pickwareAddress").val()) {
        toast("请输入取件地址~");
        return false;
    }
    if (!$("#customerContactName").val()) {
        toast("请输入联系人~");
        return false;
    }
    if (!$("#customerTel").val()) {
    ...
    //var purl = _isAfterSale == 1 ? '/Mobile/SaveRefundJdOrderAfterSales' : '/Mobile/SaveRefundOrder/';
    var purl = _isAfterSale == 1 ? '/Mobile/SaveRefundSNOrderAfterSales' : '/Mobile/SaveRefundOrder/';
        ...
        callback: function (data) {
            ...
            if (data.ResultCode == 0) {
                ...
                if (supplier == "1") {
                    ...
                    $("#app").hide();
                    $("#message").show();
                    //  window.location.href = urlAppendCommonParams("/Mobile/SnFactoryDelivery?orderId=" + $("#hidOrderId").val() + "&shopId=" + $("#hidAppId").val());
                }
                // location.reload();
            } else {
...
function JudgeSupplier() {

    var supplier = $("#supplier").val();
    //厂送
    if (supplier == "2") {
        $("#pickwareType_Li").hide();
        $("#customerContactName_Li").hide();
        $("#takePartAddressSn_Li").hide();
        $("#pickwareAddress_Li").hide();
        $("#notic").hide();
        $("#customerTel_Li").hide();
    }
    //如果等于非厂送
        ...
        //地址全部只读
        $("#customerContactName").attr("readonly", "readonly");
        $("#customerTel").attr("readonly", "readonly");
        $("#takePartAddress").attr("readonly", "readonly");
        $("#pickwareAddress").attr("readonly", "readonly");
