...
namespace Jinher.AMP.BTP.Common.Extensions
{
    public class SimpleMapper
    {
        ...
        {
            var sourceProps = sourceType.GetProperties().ToDictionary(p => p.Name);
            var destProps = destType.GetProperties().ToDictionary(p => p.Name);
            var assignments = new List<Expression>();

            foreach (var srcProp in sourceProps)
            {
                if (!srcProp.Value.GetGetMethod().IsPublic) continue;

                var destProp = destProps.ContainsKey(srcProp.Key) ? destProps[srcProp.Key] : null;
                if (destProp != null && destProp.GetSetMethod() != null)
                {
                    ...
                    if (destPropType.IsAssignableFrom(srcPropType) && destProp.GetSetMethod().IsPublic)
                    {
                        var assignmentExp = Expression.Assign(destPropExp, srcPropExp);
                        // dest.Prop = src.Prop;
                        assignments.Add(assignmentExp);
                    }
                    else if (destProp.GetGetMethod().IsPublic)
                    {
